...
try:
    ...
    data_load=pd.read_csv('Datasets/nifty_scaled.csv')
    true_data=data_load[['Open','High','Low','Close','Volume']]
...
try:
    ...
    true_dataset=TimeSeriesDataset(true_data, 'Close', lookback,shift_dict={'Close':0})

    # DataLoader to handle batching and shuffling 
    true_data_loader = DataLoader(true_dataset, batch_size=32)
